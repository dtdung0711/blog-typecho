    ...
    <div class="container typecho-dashboard">
        ...
        <div class="row typecho-page-main">
            <div class="col-mb-12 welcome-board" role="main">
                <p><?php _e('Hiện có <em>%s</em> bài viết của bạn, cùng <em>%s</em> bình luận về bạn trong <em>%s</em> danh mục.',
                        $stat->myPublishedPostsNum, $stat->myPublishedCommentsNum, $stat->categoriesNum); ?>
                    <br><?php _e('Nhấp vào các liên kết dưới đây để bắt đầu nhanh chóng:'); ?></p>

                <ul id="start-link" class="clearfix">
                    <?php if ($user->pass('contributor', true)): ?>
                        <li><a href="<?php $options->adminUrl('write-post.php'); ?>"><?php _e('Viết bài mới'); ?></a></li>
                        <?php if ($user->pass('editor', true) && 'on' == $request->get('__typecho_all_comments') && $stat->waitingCommentsNum > 0): ?>
                            <li>
                                <a href="<?php $options->adminUrl('manage-comments.php?status=waiting'); ?>"><?php _e('Bình luận đang chờ duyệt'); ?></a>
                        ...
                        <?php endif; ?>
                    <?php endif; ?>
                    <!--<li><a href="<?php $options->adminUrl('profile.php'); ?>"><?php _e('Cập nhật thông tin cá nhân của tôi'); ?></a></li>-->
                </ul>
            </div>

            <div class="col-mb-12 col-tb-4" role="complementary">
                <section class="latest-link">
                    <h3><?php _e('Bài viết mới nhất'); ?></h3>
                    ...
                    <ul>
                        <?php if ($posts->have()): ?>
                            ...
                            <?php endwhile; ?>
                        <?php else: ?>
                            <li><em><?php _e('Hiện chưa có bài viết'); ?></em></li>
                        <?php endif; ?>
                    </ul>
                </section>
            </div>
            ...
            <div class="col-mb-12 col-tb-4" role="complementary">
                <section class="latest-link">
                    <h3><?php _e('Bình luận mới nhận được'); ?></h3>
                    <ul>
                        ...
                        <?php if ($comments->have()): ?>
                            ...
                            <?php endwhile; ?>
                        <?php else: ?>
                            <li><?php _e('Hiện chưa có phản hồi'); ?></li>
                        <?php endif; ?>
                    </ul>
                </section>
            </div>
            ...
            <div class="col-mb-12 col-tb-4" role="complementary">
                <section class="latest-link">
                    <h3><?php _e('Nhật ký mới nhất từ nhà phát triển'); ?></h3>
        ...
        function applyUpdate(update) {
            if (update.available) {
                $('<div class="update-check message error"><p>'
                    + '<?php _e('Bạn đang sử dụng phiên bản %s'); ?>'.replace('%s', update.current) + '<br />'
                    + '<strong><a href="' + update.link + '" target="_blank">'
                    + '<?php _e('Phiên bản mới nhất từ nhà phát triển là %s'); ?>'.replace('%s', update.latest) + '</a></strong></p></div>')
