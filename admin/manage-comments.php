...
<div class="main">
            ...
            <div class="col-mb-12 typecho-list">
                <div class="clearfix">
                    ...
                    <?php if($user->pass('editor', true) && !isset($request->cid)): ?>
                        ...
                        <li class="<?php if(!$isAllComments): ?> current<?php endif; ?>"><a href="<?php echo $request->makeUriByRequest('__typecho_all_comments=off'); ?>"><?php _e('Bình luận của tôi'); ?></a></li>
                    <?php endif; ?>
                    </ul>
    ...
    <li<?php if(!isset($request->status) || 'approved' == $request->get('status')): ?> class="current"<?php endif; ?>><a href="<?php $options->adminUrl('manage-comments.php'
        . (isset($request->cid) ? '?cid=' . $request->filter('encode')->cid : '')); ?>"><?php _e('Đã phê duyệt'); ?></a></li>
    ...
    <div class="operate">
        ...
        <div class="btn-group btn-drop">
            <button class="btn dropdown-toggle btn-s" type="button"><i class="sr-only"><?php _e('Thao tác'); ?></i><?php _e('Các mục đã chọn'); ?> <i class="i-caret-down"></i></button>
            <ul class="dropdown-menu">
                ...
                <li><a href="<?php $security->index('/action/comments-edit?do=spam'); ?>"><?php _e('Đánh dấu là rác'); ?></a></li>
                <li><a lang="<?php _e('Bạn có chắc muốn xóa các bình luận đã chọn không?'); ?>" href="<?php $security->index('/action/comments-edit?do=delete'); ?>"><?php _e('Xóa'); ?></a></li>
            </ul>
            <?php if('spam' == $request->get('status')): ?>
                <button lang="<?php _e('Bạn có chắc muốn xóa tất cả các bình luận rác không?'); ?>" class="btn btn-s btn-warn btn-operate" href="<?php $security->index('/action/comments-edit?do=delete-spam'); ?>"><?php _e('Xóa tất cả các bình luận rác'); ?></button>
            <?php endif; ?>
        </div>
    </div>
    <div class="search" role="search">
        <?php if ('' != $request->keywords || '' != $request->category): ?>
            ...
            (isset($request->cid) ? (isset($request->status) ? '&' : '') . 'cid=' . $request->filter('encode')->cid : '') : '')); ?>"><?php _e('&laquo; Hủy lọc'); ?></a>
        <?php endif; ?>
        <input type="text" class="text-s" placeholder="<?php _e('Nhập từ khóa'); ?>" value="<?php echo $request->filter('html')->keywords; ?>"<?php if ('' == $request->keywords): ?> onclick="value='';name='keywords';" <?php else: ?> name="keywords"<?php endif; ?>/>
    ...
    <span class="weak"><?php _e('Đã phê duyệt'); ?></span>
    <?php else: ?>
    <a href="<?php $security->index('/action/comments-edit?do=approved&coid=' . $comments->coid); ?>" class="operate-approved"><?php _e('Phê duyệt'); ?></a>
    <?php endif; ?>
    ...
    <a lang="<?php _e('Bạn có chắc muốn xóa bình luận của %s?', htmlspecialchars($comments->author)); ?>" href="<?php $security->index('/action/comments-edit?do=delete&coid=' . $comments->coid); ?>" class="operate-delete"><?php _e('Xóa'); ?></a>
</div>
                            </td>
                        </tr>
                        <?php endwhile; ?>
                        ...
                        <tr>
                        <td colspan="4">
    <h6 class="typecho-list-table-title"><?php _e('Không có bình luận') ?></h6>
</td>
                        </tr>
                        <?php endif; ?>
                        </tbody>
    ...
    <label><i class="sr-only"><?php _e('Chọn tất cả'); ?></i><input type="checkbox" class="typecho-table-select-all" /></label>
    <div class="btn-group btn-drop">
        <button class="btn dropdown-toggle btn-s" type="button"><i class="sr-only"><?php _e('Thao tác'); ?></i><?php _e('Mục đã chọn'); ?> <i class="i-caret-down"></i></button>
        <ul class="dropdown-menu">
            <li><a href="<?php $security->index('/action/comments-edit?do=approved'); ?>"><?php _e('Phê duyệt'); ?></a></li>
            <li><a href="<?php $security->index('/action/comments-edit?do=waiting'); ?>"><?php _e('Chờ xác nhận'); ?></a></li>
            <li><a href="<?php $security->index('/action/comments-edit?do=spam'); ?>"><?php _e('Đánh dấu rác'); ?></a></li>
            <li><a lang="<?php _e('Bạn có chắc chắn muốn xóa các bình luận này không?'); ?>" href="<?php $security->index('/action/comments-edit?do=delete'); ?>"><?php _e('Xóa'); ?></a></li>
        </ul>
        <?php if('spam' == $request->get('status')): ?>
            <button lang="<?php _e('Bạn có chắc chắn muốn xóa tất cả bình luận rác không?'); ?>" class="btn btn-s btn-warn btn-operate" href="<?php $security->index('/action/comments-edit?do=delete-spam'); ?>"><?php _e('Xóa tất cả bình luận rác'); ?></button>
        <?php endif; ?>
    </div>
</div>
                        </div>
...
$(document).ready(function () {
    ...
    $('.operate-reply').click(function () {
        ...
        if ($('.comment-reply', td).length > 0) {
            ...
            var form = $('<form method="post" action="' + t.attr('rel') + '" class="comment-reply">' +
...
$('.cancel', form).click(function () {
    ...
    '<td colspan="2" valign="top"><form method="post" action="' + t.attr('rel') + '" class="comment-edit-info">' +
    '<p><label for="' + id + '-author"><?php _e('Tên người dùng'); ?></label><input class="text-s w-100" id="' + id + '-author" name="author" type="text"></p>' +
    ...
    '<td valign="top"><form method="post" action="' + t.attr('rel') + '" class="comment-edit-content"><p><label for="' + id + '-text"><?php _e('Nội dung'); ?></label>' +
