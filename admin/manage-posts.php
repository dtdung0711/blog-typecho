...
<div class="main">
                ...
                <div class="clearfix">
                    <ul class="typecho-option-tabs right">
                        <?php if ($user->pass('editor', true) && !isset($request->uid)): ?>
                            ...
                            <li class="<?php if (!$isAllPosts): ?> current<?php endif; ?>"><a
                                    href="<?php echo $request->makeUriByRequest('__typecho_all_posts=off&page=1'); ?>"><?php _e('Bài của tôi'); ?></a>
                            </li>
                        <?php endif; ?>
                    </ul>
                    <ul class="typecho-option-tabs">
                        ...
                        <li<?php if ('draft' == $request->get('status')): ?> class="current"<?php endif; ?>><a
                                href="<?php $options->adminUrl('manage-posts.php?status=draft'
                                    . (isset($request->uid) ? '&uid=' . $request->filter('encode')->uid : '')); ?>"><?php _e('Nháp'); ?>
    ...
    <form method="get">
        <div class="operate">
            ...
            <div class="btn-group btn-drop">
                <button class="btn dropdown-toggle btn-s" type="button"><i
                    class="sr-only"><?php _e('Thao tác'); ?></i><?php _e('Mục đã chọn'); ?> <i
                    class="i-caret-down"></i></button>
                <ul class="dropdown-menu">
                    <li><a lang="<?php _e('Bạn có chắc chắn muốn xóa các bài viết đã chọn?'); ?>"
        ...
        <div class="search" role="search">
            ...
            <select name="category">
                <option value=""><?php _e('Tất cả các danh mục'); ?></option>
            ...
            <tr id="<?php $posts->theId(); ?>">
                ...
                <td>
                    ...
                    if ('hidden' == $posts->status) {
                        ...
                        echo '<em class="status">' . _t('Bảo mật bằng mật khẩu') . '</em>';
                    }
                    ?>
                ...
                <td>
                    <?php if ($posts->hasSaved): ?>
                        <span class="description">
                            ...
                            <?php _e('Đã lưu vào %s', $modifyDate->word()); ?>
                        </span>
                    <?php else: ?>
                        <?php $posts->dateWord(); ?>
                    <?php endif; ?>
    ...
    <?php else: ?>
        <tr>
            <td colspan="6"><h6 class="typecho-list-table-title"><?php _e('Không có bài viết nào'); ?></h6>
            </td>
        </tr>
    <?php endif; ?>
</tbody>
