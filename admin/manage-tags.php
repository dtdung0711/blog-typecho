                ...
                <form method="post" name="manage_tags" class="operate-form">
                        ...
                        <div class="operate">
                            ...
                            <div class="btn-group btn-drop">
                                <button class="btn dropdown-toggle btn-s" type="button"><i
                                        class="sr-only"><?php _e('Hành động'); ?></i><?php _e('Mục được chọn'); ?> <i
                                        class="i-caret-down"></i></button>
                                <ul class="dropdown-menu">
                                    <li><a lang="<?php _e('Bạn có chắc muốn xóa các thẻ này không?'); ?>"
                                           href="<?php $security->index('/action/metas-tag-edit?do=delete'); ?>"><?php _e('Xóa'); ?></a>
                                    </li>
                                    <li><a lang="<?php _e('Làm mới thẻ có thể mất thời gian, bạn có chắc muốn làm mới các thẻ này không?'); ?>"
                                           href="<?php $security->index('/action/metas-tag-edit?do=refresh'); ?>"><?php _e('Làm mới'); ?></a>
                                    </li>
                                    <li class="multiline">
                                        <button type="button" class="btn btn-s merge"
                                                rel="<?php $security->index('/action/metas-tag-edit?do=merge'); ?>"><?php _e('Gộp vào'); ?></button>
                    ...
                    <ul class="typecho-list-notable tag-list clearfix">
                        <?php if ($tags->have()): ?>
                            ...
                            <?php endwhile; ?>
                        <?php else: ?>
                            <h6 class="typecho-list-table-title"><?php _e('Không có thẻ nào'); ?></h6>
                        <?php endif; ?>
                    </ul>
