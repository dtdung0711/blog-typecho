        ...
        <div class="row typecho-page-main" role="main">
            <div class="col-mb-12 typecho-list">
                ...
                <?php if ($activatedPlugins->have() || !empty($activatedPlugins->activatedPlugins)): ?>
                    <h4 class="typecho-list-table-title"><?php _e('Các plugin đã kích hoạt'); ?></h4>
                                ...
                                <tr id="plugin-<?php $activatedPlugins->name(); ?>">
                                    <td><?php $activatedPlugins->title(); ?>
                                        <?php if (!$activatedPlugins->dependence): ?>
                                            <i class="i-delete"
                                               title="<?php _e('%s không hoạt động đúng trên phiên bản này của Typecho', $activatedPlugins->title); ?>"></i>
                                        <?php endif; ?>
                                    </td>
                                    ...
                                    <td>
                                        <?php if ($activatedPlugins->activate || $activatedPlugins->deactivate || $activatedPlugins->config || $activatedPlugins->personalConfig): ?>
                                            <?php if ($activatedPlugins->config): ?>
                                                ...
                                                &bull;
                                            <?php endif; ?>
                                            <a lang="<?php _e('Bạn có chắc muốn vô hiệu hóa plugin %s không?', $activatedPlugins->name); ?>"
                                               href="<?php $security->index('/action/plugins-edit?deactivate=' . $activatedPlugins->name); ?>"><?php _e('Vô hiệu hóa'); ?></a>
                                        <?php else: ?>
                                            <span class="important"><?php _e('Sẵn sàng sử dụng'); ?></span>
                                        <?php endif; ?>
                                    </td>
                                </tr>
                            <?php endwhile; ?>
                            ...
                            <?php if (!empty($activatedPlugins->activatedPlugins)): ?>
                                <?php foreach ($activatedPlugins->activatedPlugins as $key => $val): ?>
                                    <tr>
                                        <td><?php echo $key; ?></td>
                                        <td colspan="3"><span
                                                class="warning"><?php _e('Tệp plugin này đã bị hỏng hoặc bị loại bỏ không an toàn, rất khuyến khích bạn vô hiệu hóa nó'); ?></span></td>
                                        <td><a lang="<?php _e('Bạn có chắc muốn vô hiệu hóa plugin %s không?', $key); ?>"
                                               href="<?php $security->index('/action/plugins-edit?deactivate=' . $key); ?>"><?php _e('Vô hiệu hóa'); ?></a>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php endif; ?>
                ...
                <?php if ($deactivatedPlugins->have() || !$activatedPlugins->have()): ?>
                    <h4 class="typecho-list-table-title"><?php _e('Plugin bị vô hiệu hóa'); ?></h4>
                    <div class="typecho-table-wrap">
                    <table class="typecho-list-table deactivate">
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="5"><h6 class="typecho-list-table-title"><?php _e('Không có plugin được cài đặt'); ?></h6>
                                    </td>
                                </tr>
                            <?php endif; ?>
                            </tbody>
