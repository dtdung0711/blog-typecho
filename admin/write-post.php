...
<div class="main">
    ...
    <div class="col-mb-12 col-tb-9" role="main">
        <?php if ($post->draft): ?>
            <?php if ($post->draft['cid'] != $post->cid): ?>
                ...
                <cite class="edit-draft-notice"><?php _e('Bạn đang chỉnh sửa bản nháp được lưu vào %s. Bạn cũng có thể <a href="%s">xóa nó</a>', $postModifyDate->word(),
                                    $security->getIndex('/action/contents-post-edit?do=deleteDraft&cid=' . $post->cid)); ?></cite>
            <?php else: ?>
                <cite class="edit-draft-notice"><?php _e('Bạn đang chỉnh sửa bản nháp chưa được xuất bản'); ?></cite>
            <?php endif; ?>
        ...
        <p class="mono url-slug">
            <label for="slug" class="sr-only"><?php _e('Tên đường dẫn'); ?></label>
        ...
        <p class="submit clearfix">
            ...
            <span class="right">
                ...
                <button type="submit" name="do" value="publish" class="btn primary"
                        id="btn-submit"><?php _e('Xuất bản bài viết'); ?></button>
    ...
    <div id="edit-secondary" class="col-mb-12 col-tb-3" role="complementary">
        <ul class="typecho-option-tabs clearfix">
            ...
            <li class="w-50"><a href="#tab-files" id="tab-files-btn"><?php _e('Tệp đính kèm'); ?></a></li>
        </ul>

        <div id="tab-advance" class="tab-content">
            <section class="typecho-post-option" role="application">
                <label for="date" class="typecho-label"><?php _e('Ngày xuất bản'); ?></label>
            ...
            <div id="advance-panel">
                        ...
                        <p>
                            <select id="visibility" name="visibility">
                                <?php if ($user->pass('editor', true)): ?>
                                    ...
                                    <option
                                        value="password"<?php if (strlen($post->password ?? '') > 0): ?> selected<?php endif; ?>><?php _e('Bảo mật bằng mật khẩu'); ?></option>
                                    <option
                                        value="private"<?php if ($post->status == 'private'): ?> selected<?php endif; ?>><?php _e('Riêng tư'); ?></option>
                                <?php endif; ?>
                                <option
                                    value="waiting"<?php if (!$user->pass('editor', true) || $post->status == 'waiting'): ?> selected<?php endif; ?>><?php _e('Chờ xem xét'); ?></option>
                            </select>
                        </p>
                        <p id="post-password"<?php if (strlen($post->password ?? '') == 0): ?> class="hidden"<?php endif; ?>>
                            <label for="protect-pwd" class="sr-only"><?php _e('Mật khẩu bài viết'); ?></label>
                            <input type="text" name="password" id="protect-pwd" class="text-s"
                                   value="<?php $post->password(); ?>" size="16"
                                   placeholder="<?php _e('Mật khẩu bài viết'); ?>" autocomplete="off"/>
                        </p>
                    </section>
                <?php endif; ?>

                <section class="typecho-post-option allow-option">
                    <label class="typecho-label"><?php _e('Kiểm soát quyền hạn'); ?></label>
                    <ul>
                        ...
                        <li><input id="allowFeed" name="allowFeed" type="checkbox" value="1"
                            ...
                            <label for="allowFeed"><?php _e('Cho phép xuất hiện trong dữ liệu cung cấp'); ?></label>
                        </li>
                    </ul>
                </section>
                ...
                <section class="typecho-post-option">
                    ...
                    <p class="description"><?php _e('Mỗi dòng một địa chỉ trích dẫn, phân cách bằng dấu xuống dòng'); ?></p>
                </section>
