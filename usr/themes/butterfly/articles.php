...
<?php endif; ?>
   ...
   <?php $this->widget('Widget_Contents_Post_Recent', 'pageSize=1000')->to($archives); Typecho_Widget::widget('Widget_Stat')->to($stat);
    ...
    $output = '<div class="article-sort-title">Tổng số bài viết - '.$stat->publishedPostsNum.' </div><div class="article-sort"> ';
    while($archives->next()):
        ...
        if ($year != $year_tmp) {
            $year = $year_tmp;
            $output .= '<div class="article-sort-item year">Năm '. $year .'</div>'; //输出年份
        }
        $output .= 
