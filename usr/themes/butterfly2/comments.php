...
<div id="comments">
    ...
    <hr></hr>
    <h3 id="response"><div class="comment-head"><div class="comment-headline"><i class="fas fa-comments fa-fw"></i><span> Bình luận</span></div></div></h3>
    <div id="<?php $this->respondId(); ?>" class="respond">
        ...
        <form method="post" action="<?php $this->commentUrl() ?>" id="comment-form" role="form">
            <div class="commments-area">
                <div class="commments-info">
                    ...
                    <div style="border-bottom: 1px dashed #dedede;">
                        <?php _e('Đăng nhập với tư cách:  '); ?>
                        <a href="<?php $this->options->profileUrl(); ?>"><?php $this->user->screenName(); ?><?php if($this->user->group == 'administrator'): ?> Quản trị viên <?php elseif($this->user->group == 'editor'): ?> Biên tập viên <?php elseif($this->user->group == 'contributor'): ?> Đóng góp viên <?php elseif($this->user->group == 'subscriber'): ?> Theo dõi <?php elseif($this->user->group == 'visitor'): ?> Khách thăm <?php endif ?></a>.</a>
                        <a href="<?php $this->options->logoutUrl(); ?>" title="Thoát"><i class="fas fa-sign-out-alt"></i></a>
                    </div>
                    <?php else: ?>
                    <label for="author" class="required"></label>
                    <input placeholder="Biệt danh" type="text" name="author" id="author" class="text" value="<?php $this->remember('author'); ?>" required />

                    <label for="mail"<?php if ($this->options->commentsRequireMail): ?> class="required"<?php endif; ?>></label>
                    <input placeholder="Email" type="email" name="mail" id="mail" class="text" value="<?php $this->remember('mail'); ?>"<?php if ($this->options->commentsRequireMail): ?> required<?php endif; ?> />
                ...
                <textarea placeholder="Bình luận của bạn" rows="8" cols="50" name="text" id="textarea" class="textarea" required ><?php $this->remember('text'); ?></textarea>
            ...
            <div class="commentsFormArea" style="text-align: right;">
                <button class="submit" type="submit" ><?php _e('Bình luận'); ?></button>
            </div>
        ...
        <div id="comment_login" style="display:none">
            <form onsubmit="return false" style="margin-top: 10px;">
                <input type="text" class="text" name="name" autocomplete="username" placeholder="Nhập tên đăng nhập" required/>
                <input type="password" class="text" name="password" autocomplete="current-password" placeholder="Nhập mật khẩu" required/>
                <button class="submit" type="submit" id="web-login">Đăng nhập</button>
            </form>
            <script>
            $("#web-login").click(function(){
                ...
                function(data){
                    if(data.search("GLOBAL_CONFIG")!= -1){
                        Dreamer.warning("Đăng nhập không thành công, vui lòng kiểm tra tài khoản mật khẩu",2000);
                    }else{
                        Dreamer.success("Đăng nhập thành công, đang chuyển hướng...",function () {
                            location.reload();       
                        });
                    }
                })
    ...
    <hr></hr>
    <h3><?php _e('Bình luận đã bị tắt'); ?></h3>
    <?php else: ?>
    <?php endif; ?>
    <?php if ($comments->have()): ?>
    <h3><?php $this->commentsNum(_t('Chưa có bình luận'), _t('Chỉ có một bình luận'), _t('Đã có %d bình luận')); ?></h3>
