...
function thumb($cid)
{
    ...
    if (empty($img)) {
        ...
        echo 'Địa chỉ blog của bạn' . $img['path'];
    }
}
...
function ArtPlayer($text)
{
    $text = preg_replace_callback('/\[video title=\"(.*?)"\ url=\"(.*?)"\ container=\"(.*?)"\ subtitle=\"(.*?)"\ poster=\"(.*?)"\](.*?)\[\/video\]/ism', function ($text) {
        ...
        $text[4] == ' ' ? $tooltip = 'Không có phụ đề' : $tooltip = 'Phụ đề mặc định';
        return '
...
function getBrowser($agent)
{
    $browsers = [
        ...
        '/Maxthon([\d]*)\/([^\s]+)/i' => ['label' => 'Trình duyệt Maxthon', 'icon' => 'iconfont icon-maxthon'],
        '#360([a-zA-Z0-9.]+)#i' => ['label' => 'Trình duyệt 360', 'icon' => 'iconfont icon-chrome'],
        '/Edg([\d]*)\/([^\s]+)/i' => ['label' => 'Microsoft Edge', 'icon' => 'fab fa-edge'],
        '/UC/i' => ['label' => 'Trình duyệt UC', 'icon' => 'iconfont icon-UCliulanqi'],
        '/QQ/i' => ['label' => 'Trình duyệt QQ', 'icon' => 'iconfont icon-QQliulanqi'],
        '/QQBrowser\/([^\s]+)/i' => ['label' => 'Trình duyệt QQ', 'icon' => 'iconfont icon-QQliulanqi'],
        '/UBrowser/i' => ['label' => 'Trình duyệt UC', 'icon' => 'iconfont icon-UCliulanqi'],
        ...
        '/XiaoMi/i' => ['label' => 'Trình duyệt Xiaomi', 'icon' => 'iconfont icon-XiaoMi'],
...
function commentRank($widget, $email = NULL)
{
    ...
    if ($widget->authorId == $widget->ownerId) {
        echo '<span class="vtag vmaster">Chủ nhà</span>';
    } else if (in_array($email, $mailList)) {
        echo '<span class="vtag vauth">' . $all[$email] . '</span>';
    } else {
        echo '<span class="vtag vvisitor">Khách</span>';
    }    
}
    ...
    if ($parent != "0") { //子评论
        ...
        if (@$author && $arow['status'] == "approved") { //父评论作者存在且父评论已经审核通过
            if (@$prow['status'] == "waiting") {
                echo '<p class="commentReview">（Bình luận đang chờ duyệt）)</p>';
            }
    ...
    if ($create >= $update) {
        ...
        foreach ($timeIntervals as $interval => $label) {
            if ($lastModified > $interval) {
                ...
                $timeElapsedString = $value . ' ' . $label . ' trước';
                break;
            }
        }
        echo $timeElapsedString;
    }
}
//文章阅读时间统计
...
function comments_filter($comment)
{
    if (isset($_REQUEST['text']) != null) {
        if ($_POST['g-recaptcha-response'] == null) {
            throw new Typecho_Widget_Exception(_t('Xác minh máy người không thành công. Hãy đảm bảo bạn đã tải mã xác minh máy người Google và đã hoàn thành xác minh.'));
        } else {
            $siteKey = Helper::options()->siteKey;
            $secretKey = Helper::options()->secretKey;
            ...
            if ($resp->success == true) {
                ...
                switch ($resp->error-codes[0]) {
                    case 'timeout-or-duplicate':
                        throw new Typecho_Widget_Exception(_t('Thời gian xác minh vượt quá 2 phút hoặc nội dung đã được gửi trước đó!'));
                        break;
                    case 'invalid-input-secret':
                        throw new Typecho_Widget_Exception(_t('Biên tập viên đã nhập siteKey hoặc secretKey không hợp lệ...'));
                        break;
                    case 'bad-request':
                        throw new Typecho_Widget_Exception(_t('Yêu cầu không hợp lệ! Vui lòng kiểm tra kết nối mạng'));
                        break;
                    default:
                        throw new Typecho_Widget_Exception(_t('Rất tiếc, bạn đã bị xem như một robot...'));
                }                
            }
        }
    }
...
function hcaptcha_filter($comment)
{
    if (isset($_REQUEST['text']) != null) {
        if ($_POST['h-captcha-response'] == null) {
            throw new Typecho_Widget_Exception(_t('Xác minh CAPTCHA thất bại, hãy đảm bảo bạn đã tích hợp thành công CAPTCHA hcaptcha và đã vượt qua xác minh.'));
        } else {
                ...
                if ($responseData->success === true || $responseData->success === 1) {
                    ...
                    switch ($responseData->error - codes) {
                        case '{[0] => "timeout-or-duplicate"}':
                            throw new Typecho_Widget_Exception(_t('Thời gian xác minh đã vượt quá 2 phút hoặc bạn đã lặp lại bình luận liên tục!'));
                            break;
                        case '{[0] => "invalid-input-secret"}':
                            throw new Typecho_Widget_Exception(_t('Quản trị viên trang web đã điền siteKey hoặc secretKey không hợp lệ...'));
                            break;
                        case '{[0] => "bad-request"}':
                            throw new Typecho_Widget_Exception(_t('Yêu cầu không hợp lệ! Vui lòng kiểm tra kết nối mạng'));
                            break;
                        default:
                            throw new Typecho_Widget_Exception(_t('Rất tiếc, bạn đã bị xem là robot...'));
                    }                    
                }
            }
        }
