...
(function () {
    ...
    private function singleHandle(Query $select, bool &$hasPushed)
    {
        ...
        if ($isPasswordPosted && $this->hidden) {
            throw new WidgetException(_t('Xin lỗi, mật khẩu bạn nhập không chính xác'), 403);
        }
    ...
    private function categoryHandle(Query $select, bool &$hasPushed)
    {
        ...
        if (isset($directory) && ($this->request->directory != implode('/', $category['directory']))) {
            throw new WidgetException(_t('Danh mục chính không tồn tại'), 404);
        }
    ...
    private function dateHandle(Query $select, &$hasPushed)
    {
        ...
        if (!empty($year) && !empty($month) && !empty($day)) {
            ...
            /** Đặt tiêu đề */
$this->archiveTitle = _t('%d năm %d tháng %d ngày', $year, $month, $day);
} elseif (!empty($year) && !empty($month)) {

    /** Nếu lưu trữ theo tháng */
    $from = mktime(0, 0, 0, $month, 1, $year);
    $to = mktime(23, 59, 59, $month, date('t', $from), $year);

    /** Tên rút gọn của lưu trữ */
    $this->archiveSlug = 'thang';

    /** Đặt tiêu đề */
    $this->archiveTitle = _t('%d năm %d tháng', $year, $month);
} elseif (!empty($year)) {

    /** Nếu lưu trữ theo năm */
    $from = mktime(0, 0, 0, 1, 1, $year);
    $to = mktime(23, 59, 59, 12, 31, $year);

    /** Tên rút gọn của lưu trữ */
    $this->archiveSlug = 'nam';
