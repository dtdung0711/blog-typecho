    ...
    private function import()
{
    ...
    if (!empty($_FILES)) {
        ...
        if (UPLOAD_ERR_OK == $file['error'] && is_uploaded_file($file['tmp_name'])) {
            ...
            Notice::alloc()->set(_t('Tải lên tệp sao lưu thất bại'), 'error');
            $this->response->goBack();
        }
    } else {
    ...
    private function extractData($file)
{
    ...
    if ($fileSize < $headerSize) {
        @fclose($fp);
        Notice::alloc()->set(_t('Định dạng tệp sao lưu không đúng'), 'error');
        $this->response->goBack();
    }
    ...
    @fclose($fp);
    Notice::alloc()->set(_t('Quá trình phục hồi dữ liệu hoàn tất'), 'success');
    $this->response->goBack();
}
    ...
    private function importData($table, $data)
{
    ...
    try {
        ...
        Notice::alloc()->set(_t('Trong quá trình khôi phục, gặp phải lỗi sau: %s', $e->getMessage()), 'error');
        $this->response->goBack();
    }
}
