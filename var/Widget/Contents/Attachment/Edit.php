...
class Edit extends PostEdit implements ActionInterface
{
    ...
    public function execute()
    {
        ...
        if (!empty($this->request->cid)) {
            $this->db->fetchRow($this->select()
                ...
                if (!$attachment) {
                    throw new Exception(_t('Tệp không tồn tại'), 404);
    ...
    _t('Tệp chưa được phân loại %s đã được cập nhật', $this->title), 'success');
        }
    ...
    public function form(): Form
    {
        ...
        $slug = new Form\Element\Text(
            ...
            $this->slug,
            _t('Slug'),
            _t('Slug của tệp được sử dụng để tạo liên kết thân thiện, khuyến nghị sử dụng chữ cái, số, gạch dưới và gạch ngang.')
        );
        $form->addInput($slug);
        ...
        $description = new Form\Element\Textarea(
            ...
            _t('Mô tả'),
            _t('Đoạn văn này được sử dụng để mô tả tệp, trong một số chủ đề nó sẽ được hiển thị.')
        );
        ...
        /** Nút gửi */
        $submit = new Form\Element\Submit(null, null, _t('Gửi chỉnh sửa'));
        $submit->input->setAttribute('class', 'btn primary');
        $delete = new Layout('a', [
            'href'  => $this->security->getIndex('/action/contents-attachment-edit?do=delete&cid=' . $this->cid),
            'class' => 'operate-delete',
            'lang'  => _t('Bạn có chắc muốn xóa tệp %s không?', $this->attachment->name)
        ]);
        $submit->container($delete->html(_t('Xóa tệp')));
        $form->addItem($submit);
        
        $name->addRule('required', _t('Bạn phải điền vào tiêu đề tệp'));
        $name->addRule([$this, 'nameToSlug'], _t('Tiêu đề tệp không thể chuyển thành Slug'));
        $slug->addRule([$this, 'slugExists'], _t('Slug đã tồn tại'));        
    ...
    public function deleteAttachment()
    {
        ...
        if ($this->request->isAjax()) {
            $this->response->throwJson($deleteCount > 0 ? ['code' => 200, 'message' => _t('Tệp đã được xóa')]
                : ['code' => 500, 'message' => _t('Không có tệp nào bị xóa')]);
        } else {
            /** Đặt thông báo */
            Notice::alloc()
                ->set(
                    $deleteCount > 0 ? _t('Tệp đã được xóa') : _t('Không có tệp nào bị xóa'),
    ...
    public function clearAttachment()
    {
        ...
        /** 设置提示信息 */
        Notice::alloc()->set(
            $deleteCount > 0 ? _t('Tệp không được phân loại đã được dọn dẹp') : _t('Không có tệp không được phân loại được dọn dẹp'),
            $deleteCount > 0 ? 'success' : 'notice'
        );        
