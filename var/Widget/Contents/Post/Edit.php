...
class Edit extends Contents implements ActionInterface
{
    ...
    public function writePost()
    {
        ...
        $contents['title'] = $this->request->get('title', _t('Tài liệu chưa đặt tên'));
    ...
    _t('Bài viết "%s" đang chờ xét duyệt', $this->title), 'success');
...
public function markPost()
{
    ...
    Notice::alloc()
        ->set(
            $markCount > 0 ? _t('Bài viết đã được đánh dấu là<strong>%s</strong>', $statusList[$status]) : _t('Không có bài viết nào được đánh dấu'),
