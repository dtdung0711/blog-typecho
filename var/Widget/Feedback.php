        ...
        if (
            ...
            if ('comment' == $callback) {
                ...
                if (!$this->content->allow('comment')) {
                    throw new Exception(_t('Xin lỗi, phản hồi cho nội dung này đã bị cấm.'), 403);
                }
                ...
                if ($this->options->commentsCheckReferer && 'false' != $this->parameter->checkReferer) {
                    ...
                    if (empty($referer)) {
                        throw new Exception(_t('Trang nguồn của bình luận không đúng.'), 403);
                    }
                ...
                if (
                    ...
                    if (
                        ...
                        throw new Exception(_t('Xin lỗi, bạn đã đăng bình luận quá thường xuyên. Vui lòng đợi một chút trước khi đăng lại.'), 403);
                    }
                }
            }
            ...
            if ('trackback' == $callback && !$this->content->allow('ping')) {
                throw new Exception(_t('Xin lỗi, tham chiếu đến nội dung này đã bị cấm.'), 403);
            }
    ...
    private function comment()
    {
        ...
        if ($parentId = $this->request->filter('int')->get('parent')) {
            if (
                ...
                throw new Exception(_t('Bình luận cha không tồn tại'));
            }
        }

        //检验格式
        $validator = new Validate();
        $validator->addRule('author', 'required', _t('Bạn phải điền tên người dùng'));
        $validator->addRule('author', 'xssCheck', _t('Vui lòng không sử dụng các ký tự đặc biệt trong tên người dùng'));
        $validator->addRule('author', [$this, 'requireUserLogin'], _t('Tên người dùng bạn đang sử dụng đã được đăng ký, vui lòng đăng nhập và thử lại'));
        $validator->addRule('author', 'maxLength', _t('Tên người dùng tối đa có thể chứa 150 ký tự'), 150);
        ...
        $validator->addRule('mail', 'maxLength', _t('Địa chỉ email tối đa có thể chứa 150 ký tự'), 150);        

        if ($this->options->commentsRequireUrl && !$this->user->hasLogin()) {
            $validator->addRule('url', 'required', _t('Bạn phải điền vào trang cá nhân'));
        }
        $validator->addRule('url', 'url', _t('Định dạng địa chỉ trang cá nhân không đúng'));
$validator->addRule('url', 'maxLength', _t('Địa chỉ trang cá nhân tối đa có thể chứa 255 ký tự'), 255);

$validator->addRule('text', 'required', _t('Bạn phải điền nội dung bình luận'));
    ...
    private function trackback()
    {
        ...
        if (
            ...
            /** 使用403告诉机器人 */
            throw new Exception(_t('Không được phép gửi biểu mẫu nhiều lần'), 403);
        }
